...
const Header = () => {
  ...
  return (
    <>
      <header
        className={`header left-0 top-1 z-40 flex w-full items-center  h-24 pb-2 ${sticky
                        ...
                        {menuItem.path ? (
                          <Link
                            ...
                            className={`flex py-2   text-lg lg:mr-0 lg:inline-flex lg:px-0 lg:py-6 ${usePathName === menuItem.path
                              ? "text-primary "
                              : ` ${!sticky && usePathName.includes("/services/webdevelopment") ? "text-black" : ""}    hover:cursor-pointer`
                              }`}
                          >
                            {menuItem.title}
                          </Link>
                          ...
                          <>
                            <p
                              ...
                              className={`${!sticky && usePathName.includes("/services/webdevelopment") ? "text-black" : ""} flex cursor-pointer font-bolditems-center justify-between py-2 text-lg text-dark group-hover:text-primary  lg:mr-0 lg:inline-flex lg:px-0 lg:py-6 `}
                            ...
                            <div
                              // className={` submenu relative left-0 top-full rounded-sm bg-white transition-[top] duration-300 group-hover:opacity-100 dark:bg-dark lg:invisible lg:absolute lg:top-[110%] lg:block lg:w-max lg:p-4 lg:opacity-0 lg:shadow-lg lg:group-hover:visible lg:group-hover:top-full ${openIndex === index ? "block" : "hidden"
                              //   }`}
                              className={`bg-white flex gap-4  text-lg submenu relative left-0 top-full rounded-2xl   transition-[top] duration-300 group-hover:opacity-100 dark:bg-dark lg:invisible lg:absolute lg:top-[110%] lg:flex lg:w-max lg:p-4 lg:opacity-0 lg:shadow-lg lg:group-hover:visible lg:group-hover:top-full ${openIndex === index ? "block" : "hidden"}`}
                            >
                              <div className=" text-gray-700">
                              {menuItem.submenu && menuItem.submenu.slice(0, 8).map((submenuItem, index) => {
  return (
    <Link
    ...
    >
      {submenuItem.title}
                              ...
                              <div className=" text-gray-700">
                              {menuItem.submenu && menuItem.submenu.slice(8).map((submenuItem, index) => {
  return (
    <Link
      href={submenuItem.path}
      key={index}
